...
protected:
    ...
    /// Вспомогательный метод - получение последнего узла списка.
    Node<T>* getLastNode();

    /// Вспомогательный метод - перемещение узла из списка
    Node<T>* removeNodeFromOther(Node<T>* prevNode);

    /// Вспомогательный метод - обмен данными между двумя списками
    void swap(LinkedList<T>& linkedList);
